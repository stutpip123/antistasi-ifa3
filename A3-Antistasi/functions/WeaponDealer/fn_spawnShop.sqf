...
switch (_shopSize) do
{
    case (1):
    {
        _assets =
        [
            ["Land_CampingTable_F", [-2, 2.2, 0], 180],
            ["Land_CampingTable_F", [-0.5, -2.2, 0], 0],
            ["Land_CampingTable_small_F", [2.5, 2.2, 0], 180],
            ["Box_NATO_Equip_F", [-2.5, -2.2, 0], 90]
        ];
    };
    case (2):
    {
        ...
        [
            ["Land_CampingTable_F", [-2, 2.2, 0], 180],
            ["Land_CampingTable_F", [-0.5, -2.2, 0], 0],
            ["Land_CampingTable_F", [4.5, -2.2, 0], 0],
            ["Land_CampingTable_small_F", [2.5, 2.2, 0], 180],
            ["Box_NATO_Equip_F", [-2.5, -2.2, 0], 90]
        ];
    };
    case (3):
    {
        ...
        [
            ...
            ["Land_CampingTable_F", [4.5, -2.2, 0], 0],
            ["Land_CampingTable_small_F", [2.5, 2.2, 0], 180],
            ["Land_MapBoard_01_Wall_F", [0.5, 2.6, 1], 180],
            ["Land_MapBoard_01_Wall_F", [2, -2.6, 1], 0],
            ["Box_NATO_Equip_F", [-2.5, -2.2, 0], 90]
        ];
    };
};
...
private _fnc_getSlotPositions =
{
    ...
    switch (typeOf _asset) do
    {
        ...
        {
            private _pos = (getPos _asset) vectorAdd [0, 0, 1];
        ...
        case ("Land_CampingTable_F"):
        {
            ...
            private _pos = (getPos _asset) vectorAdd (_tableSide vectorMultiply -0.5) vectorAdd [0, 0, 1];
            private _rot = [150 + (getDir _asset), 0, 0];
            _result pushBack [_asset, _pos, _rot];
            _pos = (getPos _asset) vectorAdd (_tableSide vectorMultiply 0.5) vectorAdd [0, 0, 1];
            _result pushBack [_asset, _pos, _rot];
        };
        case ("Land_MapBoard_01_Wall_F"):
        {
            private _pos = (getPos _asset) vectorAdd ((vectorDir _asset) vectorMultiply -0.15) vectorAdd [0, 0, 0.6];
...
private _garagePos = getPos _garage;
...
{
    private _arrow = "SatchelCharge_F" createVehicle _garagePos;
    _arrow setPos (_x#1);
    [_arrow, _x#2] call BIS_fnc_setObjectRotation;
    _arrow setDamage 1;
    _arrow addAction ["Buy weapon", {hint "Weapon bought!";}];
} forEach _slots;
